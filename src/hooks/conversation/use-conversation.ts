...
export const useConversation = () => {
  ...
  useEffect(() => {
    const search = watch(async (value) => {
      ...
      try {
        ...
        console.log('[DEBUG] onGetDomainChatRooms returned:', rooms)
        if (rooms) {
          setLoading(false)
          setChatRooms(rooms.customer)
          console.log('[DEBUG] setChatRooms:', rooms.customer)
        }
      } catch (error) {
        console.log(error)
      }
  ...
  useEffect(() => {
    // Subscribe to Pusher for real-time chat updates
    const channel = pusherClient.subscribe('chat-messages')
    channel.bind('new-message', () => {
      // Re-fetch chat rooms when a new message is received
      watch(async (value) => {
        setLoading(true)
        try {
          const rooms = await onGetDomainChatRooms(value.domain)
