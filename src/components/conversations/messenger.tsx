    ...
    <div className="flex-1 flex flex-col h-0 relative">
      <div className="flex-1 h-0 w-full flex flex-col">
        <Loader loading={loading}>
          {/* Show Real Time badge if any message contains (realtime) or chat is in live mode */}
          {chats.some(chat => chat.message?.includes('(realtime)')) && (
            <div className="px-3 rounded-full py-1 bg-orange font-bold text-white text-sm w-fit mb-2 ml-2">Real Time</div>
          )}
          <div
          ...
          >
            {chats.length ? (
              chats.map((chat, idx) => (
                <Bubble
                  key={
                    chat.id && chat.id !== 'user' && chat.id !== 'assistant'
                      ? chat.id
                      : `${chat.role}-${chat.createdAt}-${idx}`
                  }
                  message={{
                    role: chat.role!,
